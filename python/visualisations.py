...
def make_handover_delay_results_table(RESULTS_DF: pd.DataFrame) -> pd.DataFrame:
    """
    Creates the handover delay results table using the RESULTS_DF from the modelling, with the results showing as "values +/- error".
    ...
    """
    results_df = RESULTS_DF[['time_now_3', 
                             'pred_3', 
    ...
    results_df['3hr-Prediction (minutes)'] = results_df['pred_3'].astype(str) + ' ' + u"\u00B1" + ' ' + results_df['mean_absolute_error_3'].astype(str)
    results_df['10hr-Prediction (minutes)'] = results_df['pred_10'].astype(str) + ' ' + u"\u00B1" + ' ' + results_df['mean_absolute_error_10'].astype(str)
    results_df['24hr-Prediction (minutes)'] = results_df['pred_24'].astype(str) + ' ' + u"\u00B1" + ' ' + results_df['mean_absolute_error_24'].astype(str)
    ...
    results_table = results_table.rename(columns={'3hr-Prediction (minutes)': 'Predicted Handover Delay in 3hr (minutes)',
                                                  '10hr-Prediction (minutes)': 'Predicted Handover Delay in 10hr (minutes)',
                                                  '24hr-Prediction (minutes)': 'Predicted Handover Delay in 24hr (minutes)'})
...
def make_handover_times_results_table(RESULTS_DF: pd.DataFrame, coloured_output: bool = True, highlight_by: str = 'mean') -> pd.DataFrame:
    """ 
    ...
    highlight_by: str
        Can be either of "mean" or "lower_bound", depending on the desired condition which is to be used for the cell highlighting.
    ...
    for enum, col_name in enumerate(lower_bound_cols):
        HANDOVER_TIMES_DF[col_name] = HANDOVER_TIMES_DF[handover_time_cols[enum]] - HANDOVER_TIMES_DF[mean_absolute_error_cols[enum]]
        HANDOVER_TIMES_DF[upper_bound_cols[enum]] = HANDOVER_TIMES_DF[handover_time_cols[enum]] + HANDOVER_TIMES_DF[mean_absolute_error_cols[enum]]
    ...
    for enum, col_name in enumerate(handover_time_range_cols):
        HANDOVER_TIMES_DF[col_name] = HANDOVER_TIMES_DF[lower_bound_cols[enum]].astype(str) + " - " + HANDOVER_TIMES_DF[upper_bound_cols[enum]].astype(str)
    ...
    HANDOVER_TIMES_TABLE= HANDOVER_TIMES_TABLE.rename(columns={'handover_time_range_3hr': 'Handover Time Range for 3hr (minutes)', 
                                                               'handover_time_range_10hr': 'Handover Time Range for 10hr (minutes)', 
                                                               'handover_time_range_24hr': 'Handover Time Range for 24hr (minutes)'})
    if coloured_output:
        ...
        # otherwise, add the important features of each prediction interval to their corresponding hospital
            ...
            for hospital_features in v:
                for enum, feature in enumerate(hospital_features):
                    important_features_df.loc[(list_of_hospitals[hospital_counter], enum+1), pred_cols[col_count]] = feature
                hospital_counter += 1
            col_count += 1
            hospital_counter = 0
