...
def generate_fake_assignments_data(start_timestamp:str, end_timestamp:str, number_of_rows:int):
    ...
    # Creating a dataframe where columns are made from randomly choosing times using the random_times function or randomly selecting values from pre-defined lists
    ...
    df['haloing_time_mins'] = np.where(df['haloing_done'], ((df['time_handover'] - df['time_clear']).dt.total_seconds() / 60).round(2), np.nan)
