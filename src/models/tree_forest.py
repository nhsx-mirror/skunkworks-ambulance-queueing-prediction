...
from sklearn.preprocessing import FunctionTransformer, OrdinalEncoder, KBinsDiscretizer, LabelEncoder, OneHotEncoder
...
def prepare_train_test_split(df: pd.DataFrame):
    ...
    feature_variables_train, feature_variables_test, target_variables_train, target_variables_test
        Subsets of the data for the feature and target variables which are to be used the training and testing process of the model. 
    """
    ...
    feature_variables_train, feature_variables_test, target_variables_train, target_variables_test = train_test_split(feature_variables, target_variables, test_size=0.20, random_state=0)
...
def tree_nodes_as_df(clf: BaseDecisionTree, id_offset=0, names=True):
    """
    Inspects the tree and outputs a dataframe of nodes of all the trees describing the split points and their impurities.
...
def forest_nodes_as_df(clf: BaseForest, names=True) -> pd.DataFrame:
    """
    Inspects the forest and outputs a dataframe of nodes of all the trees describing the split points and their impurities.
...
def loop_decision_tree_model(ORDINALS, REALS, datasets_dict: dict, list_of_hospitals: list, feature_importance: bool = True):
    """
    ...
    for hospital in list_of_hospitals:
        ...
        for hour, df_X in datasets_dict.items():
            ...
            # train-test split
            feature_variables_train, feature_variables_test, target_variables_train, target_variables_test = prepare_train_test_split(hospital_subset)

            # run model
            feature_subset, dtr, preprocessed, tree_model = train_decision_tree_model(ORDINALS, REALS, target_variables_train, feature_variables_train[[*ORDINALS.keys(), *REALS]])
            ...
            # get the timestamp at which the model was ran to get the results (now) after 3-, 10-, and 24- hours.
            if hour == '3':
                latest_timestamp = pd.to_datetime(hospital_subset.time.iloc[-1], format="%Y-%m-%d %H:%M:%S") - timedelta(hours=3)
            elif hour == '10':
                latest_timestamp = pd.to_datetime(hospital_subset.time.iloc[-1], format="%Y-%m-%d %H:%M:%S") - timedelta(hours=10)
            elif hour == '24':
                latest_timestamp = pd.to_datetime(hospital_subset.time.iloc[-1], format="%Y-%m-%d %H:%M:%S") - timedelta(hours=24)

            # add the results to the temporary table
            df_temp.loc[hospital, 'time_now_' + hour] = latest_timestamp
            df_temp.loc[hospital, 'pred_' + hour] = target_variables_pred[-1]
            df_temp.loc[hospital, 'mean_absolute_error_' + hour] = mean_absolute_error(target_variables_pred, target_variables_test)
            df_temp.loc[hospital, 'root_mean_squared_error_' + hour] = sqrt(mean_squared_error(target_variables_pred, target_variables_test))
            ...
            if feature_importance:
                ...
                RESULTS_DICT = get_important_features('decision tree', RESULTS_DICT, feature_variables_test, ORDINALS, REALS, dtr, preprocessed, feature_subset, 3, hour)
...
def loop_random_forest_model(ORDINALS, REALS, datasets_dict: dict, list_of_hospitals: list, feature_importance: bool = True):
    """
    ...
    for hospital in list_of_hospitals:
        ...
        for hour, df_X in datasets_dict.items():
            ...
            # run model
            feature_subset, rfr, preprocessed, forest_model = train_random_forest_model(ORDINALS, REALS, target_variables_train, feature_variables_train[[*ORDINALS.keys(), *REALS]])
            ...
            if feature_importance:
                ...
                RESULTS_DICT = get_important_features('random forest', RESULTS_DICT, feature_variables_test, ORDINALS, REALS, rfr, preprocessed, feature_subset, 3, hour)
...
def get_important_features(model_name: str, RESULTS_DICT: dict, feature_variables_test: pd.DataFrame, ORDINALS, REALS, regressor_variable, preprocessed_variable, feature_subset, n_important_features: int, hour: str):
    """
    ...
    if 'forest' in model_name.lower().replace(' ', ''):
        dp, _ = regressor_variable.decision_path(preprocessed_variable.transform(feature_variables_test[[*ORDINALS, *REALS]]))
    elif 'tree' in model_name.lower().replace(' ', ''):
        dp = regressor_variable.decision_path(preprocessed_variable.transform(feature_variables_test[[*ORDINALS, *REALS]]))
    ...
    df_features = pd.DataFrame([enum, feature] for enum, feature in enumerate(feature_subset)).rename(columns={0: 'parent_feature', 
                                                                                                               1: 'parent_feature_name'})

    df = df.reset_index().merge(df_features, on='parent_feature', how='left')

    df_grouped = df.groupby(['observation','parent_feature_name']).sum().sort_values(['observation','feature_importance'], ascending=[True, False])
