...
import numpy as np
import pandas as pd
from data.acquisition import get_bank_holiday_data, get_weather_data
...
def skills_by_crew_member(assignments_and_incidents_merged: pd.DataFrame):
    ...
    s = df_skill.melt(id_vars=['time', 'hospital'],
                      var_name='crew',
                      value_name='skill').set_index(['hospital',
                                                     'time', 'crew'])['skill'].dropna()
    # Split the crew skills where a comma appears and assigns it a True value,
    # stacks arrays in sequence horizontally and fill missing values with NA
    df_skills = pd.DataFrame(np.hstack(s.str.split(',')
                                       .apply(lambda words: {w.strip(): True for w in words}))
                             .tolist(),
                             index=s.index).fillna(False)
    return df_skills
...
def past_events(cleaned_assignments: pd.DataFrame, cleaned_incidents: pd.DataFrame, cleaned_pts: pd.DataFrame):
    ...
    responding_priority = responding_priority.pivot_table(index=['hospital', 'time'],
                                                          columns='responding_priority',
                                                          aggfunc=len,
                                                          fill_value=0)
    # change value of field before pivot columns=lambnda d: priority_d int d
    responding_priority = responding_priority.rename(columns={0.0: 'priority_0', 1.0: 'priority_1',
                                                              2.0: 'priority_2', 3.0: 'priority_3',
                                                              4.0: 'priority_4', 5.0: 'priority_5',
                                                              6.0: 'priority_6', 7.0: 'priority_7',
                                                              8.0: 'priority_8', 9.0: 'priority_9'})
    ...
    num_patients_transported = num_patients_transported.pivot_table(index=['hospital', 'time'],
                                                                    columns='num_patients_transported',
                                                                    aggfunc=len, fill_value=0)
    num_patients_transported = num_patients_transported.rename(columns={1: '1_patient_transported',
                                                                        2: '2_patients_transported',
                                                                        3: '3_patients_transported'})
    ...
    missing_skills = missing_skills.groupby(['hospital', 'time'])\
        ...
        .astype(int)\
        .add_prefix('missing_')
    # Re-naming handover delay to past delay to use in subsequent function
    # to calculate the average past delays over a 6 hour window
    past_delays = df[['hospital', 'time', 'handover_delay_mins']] \
        .rename(columns={'handover_delay_mins': 'past_delay_mins'})
    past_delays = past_delays.groupby(['hospital', 'time']).mean()
    # Joining all above dataframes to create one dataframe
    past_events = num_amb_arrivals.join([missing_skills,
                                         cleaned_pts,
                                         past_delays,
                                         age_band,
                                         responding_priority,
...
def prepare_delays(cleaned_assignments: pd.DataFrame):
    ...
    # Aim to predict at 3 hours, 10 hours, 24 hours, duplicated columns
    # and re-named in order to merge in subsequet functions
...
def distances_between_hospitals(cleaned_assignments_df: pd.DataFrame):
    ...
    # Adding a dummy column to merge the dataframe to itself, adding _other
    # as a suffix to calculate the Euclidean distance between each hospital
    df_easting_northing['dummy'] = 1
    df_easting_northing = df_easting_northing.merge(df_easting_northing, on='dummy',
                                                    suffixes=(None, '_other')).drop(columns='dummy')
    df_easting_northing['distance'] = np.sqrt(np.square
                                              (df_easting_northing.dest_easting
                                               - df_easting_northing.dest_easting_other)
                                              + np.square
                                              (df_easting_northing.dest_northing
                                               - df_easting_northing.dest_northing_other))
    df_distance = df_easting_northing[['hospital', 'hospital_other', 'distance']]
    return df_distance


def rolling_sums_and_means(hospital: str,
                           delays_at_hospital: pd.DataFrame,
    ...
    added_hours = timedelta(hours=hours)
    ...
    df_other_hospitals_dist = other_hospitals_df.merge(distances_at_hospital,
                                                       left_on='hospital_other',
                                                       right_on='hospital',
                                                       how='left').drop(columns='hospital')
    df_other_hospitals_dist = df_other_hospitals_dist[['hospital_other',
                                                       'time_other',
                                                       'num_ambulance_arrivals_other',
                                                       'distance']]
    # Adding weighted arrivals column which is an exponentially weighted sum of the number of
    # ambulances arriving at other hospitals depending on the distance to the local hospital
    df_other_hospitals_dist['weighted_arrivals_other'] = df_other_hospitals_dist['num_ambulance_arrivals_other'] * \
        np.exp(-(np.log(2) / 50000) * df_other_hospitals_dist['distance'])
    ...
    all_hospitals_df = pd.concat([all_hospitals, delays_at_hospital]).set_index(['time_plus_' + str(hours)]). \
        sort_values('time_plus_' + str(hours))
